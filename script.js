...
$("#search-btn").on("click", function (e) {
  ...
  $.ajax({
    ...
    $.ajax({
      ...
      $("#uv-index").css(
        "background-color",
        uvColors[Math.floor(response.value)]
      );
    });
    ...
    $.ajax({
      ...
      $("#wind-speed").text(
        Math.round(parseInt(response.list[0].wind.speed) * 2.2369 * 10) / 10 +
          " MPH"
      );
      ...
      $.ajax({
        ...
        for (var i = 0; i < response.list.length; i++) {
          if (response.list[i].dt_txt.indexOf("15:00:00") !== -1) {
            ...
            tempArray[k].textContent =
              Math.round(
                ((parseInt(response.list[i].main.temp) - 273.15) * (9 / 5) +
                  32) *
                  10
              ) /
                10 +
